...
def run_pipeline(
    ...
    *,
    ...
    privacy_candidates = audit_candidates(sanitized)
    ...
    (output_dir / "emotional-memory-profile.md").write_text(render_plain_artifact("Emotional Memory Profile"), encoding="utf-8")
    (output_dir / "relationship-texture.md").write_text(render_plain_artifact("Relationship Texture"), encoding="utf-8")
    (output_dir / "scenario-response-guide.md").write_text(render_plain_artifact("Scenario Response Guide"), encoding="utf-8")
    ...
    (output_dir / "session-memory.md").write_text(render_plain_artifact("Companion Session Memory"), encoding="utf-8")
    ...
    (output_dir / "bundle-index.md").write_text(
        ...
        "## Readiness\n\n"
        "- State: draft\n"
        "- Private companion use: review recommended before activation\n"
        "- Publishable: no\n\n"
        "## Input Summary\n\n"
        f"- Privacy mode: {mode}\n"
        f"- Date range: {analysis.get('first_timestamp')} to {analysis.get('last_timestamp')}\n"
